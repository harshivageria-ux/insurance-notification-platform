...
@Component({
  ...
  template: `
    <div class="container">
      <div class="header">
        <h1>Templates</h1>
        <button class="btn-add" (click)="openModal()">+ Add Template</button>
      </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr *ngFor="let item of paginatedItems">
            <td>{{item.id}}</td>
            <td>{{item.name}}</td>
            <td>{{item.content | slice:0:30}}{{item.content.length > 30 ? '...' : ''}}</td>
            <td>{{item.template_group_id}}</td>
            <td>
      ...
      <div class="modal" *ngIf="showModal" (click)="showModal = false">
        <div class="modal-content large" (click)="$event.stopPropagation()">
          ...
          <div class="modal-body">
            ...
            <input type="number" placeholder="Template Group ID" [(ngModel)]="newItem.template_group_id">
            <textarea placeholder="Content" [(ngModel)]="newItem.content" rows="6"></textarea>
            <input type="text" placeholder="Variables (JSON)" [(ngModel)]="newItem.variables">
          </div>
  ...
  styles: [`
    ...
    .modal { display: flex; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); justify-content: center; align-items: center; z-index: 1000; }
    .modal-content { background: white; padding: 30px; border-radius: 8px; width: 500px; }
    .modal-content.large { width: 700px; height: 80vh; overflow-y: auto; }
    input, textarea, select { width: 100%; padding: 10px; margin-bottom: 15px; border: 1px solid #ddd; font-family: inherit; }
...
export class TemplatesComponent implements OnInit {
  ...
  newItem: any = { name: '', content: '', template_group_id: 1, variables: '{}' };
  ...
  openModal() {
    ...
    this.newItem = { name: '', content: '', template_group_id: 1, variables: '{}' };
  }
