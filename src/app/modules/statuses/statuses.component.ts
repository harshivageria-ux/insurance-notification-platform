...
@Component({
  ...
  template: `
    <div class="container">
      <div class="header">
        <h1>Statuses</h1>
        <button class="btn-add" (click)="openModal()">+ Add Status</button>
      </div>
      ...
      <div class="modal" *ngIf="showModal" (click)="showModal = false">
        <div class="modal-content" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h2>{{isEditMode ? 'Edit Status' : 'Add Status'}}</h2>
            <button class="close" (click)="showModal = false">×</button>
          </div>
          <div class="modal-body">
            <input type="text" placeholder="Name" [(ngModel)]="newItem.name">
            <input type="text" placeholder="Description" [(ngModel)]="newItem.description">
...
export class StatusesComponent implements OnInit {
  ...
  newItem: any = { name: '', description: '', status: 'Active' };
  ...
  openModal() {
    ...
    this.newItem = { name: '', description: '', status: 'Active' };
  }
